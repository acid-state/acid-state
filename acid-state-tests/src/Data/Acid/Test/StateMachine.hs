...
lookupMethod :: (Acid.Method m, Acid.IsAcidic (Acid.MethodState m))
             => m -> State (Acid.MethodState m) (Acid.MethodResult m)
lookupMethod m = Acid.lookupHotMethod mmap m
  where
    mmap = Acid.mkMethodMap (Acid.eventsToMethods Acid.acidEvents)
